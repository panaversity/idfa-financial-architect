...
formula, in correct IDFA notation — *before* writing anything to Excel.
...
to verify by reading back each affected Named Range via `read_cell()`. The
reported answer and the verified answer must match exactly — not approximately.

**Pass criteria:** Agent uses `write_cell()` → `read_cell()` sequence for
...
Ask it to produce a full IDFA Logic Map via `inspect_model()`. Ask a human
...
EBITDA to equal exactly $3M"). The agent must iterate `write_cell()` and
`read_cell()` until the target is reached. It must not estimate or calculate
the answer internally.

**Pass criteria:** Agent reaches target through MCP iteration; final
`read_cell()` confirms the output equals the target; agent reports the
required input value from the model, not from calculation.
...
iterate via MCP, collect results, and return a distribution summary
(mean, median, 10th percentile, 90th percentile).

**Pass criteria:** 500 MCP iterations completed; results collected from
`read_cell()` after each iteration; distribution statistics calculated from
the collected results, not estimated; no internal simulation performed.
...
inspect_model()
```
Produces: full list of Named Ranges (if any exist), all formulas, all
hardcoded values, dependency map showing which cells feed which others.
...
1. Read the original formula via `read_formula()`
...
*IDFA Reference Guide v1.0 — Panaversity (https://panaversity.org)*
